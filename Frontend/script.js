...
function updateBoardDisplay(data) {
    ...
    columns.forEach((colId, index) => {
        ...
        tasks.forEach(task => {
            ...
            taskInfo.textContent = `Task ${task.id} (${task.created_at}) (status: ${task.status}) (Worker: ${task.worker_task}) (Cycle Time: ${task.cycle_time})`;
...
updateBtn.addEventListener('click', async function() {
    ...
    if (newconfig.column_0 < 1 || newconfig.column_1 < 1 || newconfig.column_2 < 1 || newconfig.column_3 < 1 || newconfig.column_4 < 1 || newconfig.column_5 < 1 || newconfig.workers_1 < 1 || newconfig.workers_2 < 1 || newconfig.workers_3 < 1 || newconfig.workers_4 < 1 || newconfig.tick_interval < 0.1) {
